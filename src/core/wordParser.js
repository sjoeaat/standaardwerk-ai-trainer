// src/core/wordParser.js - Enhanced versie met volledige parsing
...
function convertToStandaardwerkFormat(rawContent, syntaxRules) {
  ...
  for (let line of lines) {
    ...
    const stepMatch = allStepWords.find(kw => upper.startsWith(kw + ' '));
    const rustMatch = allRestWords.find(kw => upper.startsWith(kw));
  
    // Detectie STAP X: of SCHRITT X:
    const stepRegex = new RegExp(`^(${allStepWords.join('|')})\\s+(\\d+):?\\s*(.*)`, 'i');
    const rustRegex = new RegExp(`^(${allRestWords.join('|')}):?\\s*(.*)`, 'i');
  
    const step = stepRegex.exec(line);
    const rust = rustRegex.exec(line);
  
    if (step) {
      const [, keyword, number, description] = step;
      output.push(`${keyword.toUpperCase()} ${number}: ${description}`);
      insideStep = true;
    } else if (rust) {
      const [, keyword, description] = rust;
      output.push(`${keyword.toUpperCase()}: ${description}`);
      insideStep = true;
    } else if (insideStep) {
      output.push(`\t${line}`);
    } else {
      output.push(line); // niet ingesprongen buiten stappen
    }
  }
